...
int GraphType<VertexType>::OutDegree(VertexType v) {
    ...
    int row = IndexIs(vertices, v), val = 0;

    for ( int i = 0; i < maxVertices; i++) {
        if( edges[row][i] != NULL_EDGE && row != i)
            val++;
    }
    return val;
}
...
// Modified BFS to find the length
template<class VertexType>
int
GraphType<VertexType>::BFS_to_find_length(VertexType startVertex, VertexType endVertex)
{
    int length[maxVertices];
    for ( int i =0; i < maxVertices; i++) {
        length[i] = 0;
    }
    QueType<VertexType> queue;
    ...
    {
        queue.Dequeue(vertex);
        int parent = length[IndexIs(vertices, vertex)];
        if (vertex == endVertex)
        {
            return parent;
        }
